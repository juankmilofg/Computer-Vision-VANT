...
ORB_VANT::ORB_VANT(Mat *img1,Mat *img2)
{
    this->imagem1 = *img1;
    this->imagem2 = *img2;
    ...
    Ptr<ORB> orb =  ORB::create(600,1.2f,8,10,0,2,ORB::FAST_SCORE, 31); //ORB::HARRIS_SCORE //FAST_SCORE
    Ptr<ORB> orb1 = ORB::create(6000,1.2f,8,10,0,2,ORB::FAST_SCORE, 31); 
    ...
    orb->detectAndCompute(imagem1, noArray(), keypoints1, descriptors1);   
    ...
    //Imagem busqueda
    gettimeofday(&start, NULL);
    orb1->detectAndCompute(imagem2, noArray(), keypoints2, descriptors2);
    gettimeofday(&end, NULL);
    seconds  = end.tv_sec  - start.tv_sec;
    useconds = end.tv_usec - start.tv_usec;
...
long ORB_VANT::tempo_1( )
{
  return tempo1;
}

long ORB_VANT::tempo_2( )
{
  return tempo2;
}

long ORB_VANT::tempo__casamento( )
{
  return tempo_casamento;
}

int ORB_VANT::pontos_1( )
{
  return pontos1;
}

int ORB_VANT::pontos_2( )
{
  return pontos2;
}

int ORB_VANT::pontos_casamento( )
{
  return pontos_matches; 
}

int ORB_VANT::pontos_casamento_total( )
{
  return pontos_matches_total;
}

vector<KeyPoint> ORB_VANT::key1( )
{
  return keypointsfil11;
}

vector<KeyPoint> ORB_VANT::key2( )
{
...
vector< DMatch > ORB_VANT::keymatches( )
{
  return matches;
}
