...
ASURF_VANT::ASURF_VANT(Mat *img1,Mat *img2)
{
    this->imagem1 = *img1;
    this->imagem2 = *img2;
    ...
    for (tt = 1; tt <= num_of_tilts2; tt++)
    {
        ...
        {					
            ...
            //-- Step 1: Detect the keypoints using SIFT Detector
            vector<KeyPoint> keypoints1;
            surf->detect( imagem2, keypoints1 );
            
            //-- Step 2: Calculate descriptors (feature vectors)
            Mat descriptors1;
            surf->compute( imagem2, keypoints1, descriptors1 );
...
long ASURF_VANT::tempo_1( )
{
  return tempo1;
}

long ASURF_VANT::tempo_2( )
{
  return tempo2;
}

long ASURF_VANT::tempo__casamento( )
{
  return tempo_casamento;
}

int ASURF_VANT::pontos_1( )
{
  return pontos1;
}

int ASURF_VANT::pontos_2( )
{
  return pontos2;
}

int ASURF_VANT::pontos_casamento( )
{
  return pontos_matches; 
}

int ASURF_VANT::pontos_casamento_total( )
{
  return pontos_matches_total;
}

vector<KeyPoint> ASURF_VANT::key1( )
{
  return keypointsfil11;
}

vector<KeyPoint> ASURF_VANT::key2( )
{
  return keypointsfil22;
}

Mat ASURF_VANT::desc1( )
{
  return descriptorsfil11;
}

Mat ASURF_VANT::desc2( )
{
  return descriptorsfil22;
}

vector< DMatch > ASURF_VANT::keymatches( )
{
  return matches;
}
