...
AAKAZE_ANGULOS_VANT::AAKAZE_ANGULOS_VANT(Mat *img1,Mat *img2, int theta, int phi)
{
    this->imagem1 = *img1;
    this->imagem2 = *img2;
        ...
        if ((k==0)||(Angulotheta[AngulothetaIntervalo[k]]!= Angulotheta[AngulothetaIntervalo[0]]))
        {
            ...
            { 
                if ((j==0)||(Angulophi[AngulothetaIntervalo[k]][AngulophiIntervalo[k][j]]!=Angulophi[AngulothetaIntervalo[k]]                               [AngulophiIntervalo[k][0]]))
                {
                    ...
                    akaze->detect( transformada, keypoints1  );

                    //-- Step 2: Calculate descriptors (feature vectors)
                    Mat descriptors1;
                    akaze->compute( transformada, keypoints1 , descriptors1 ); 
    ...
    gettimeofday(&start, NULL);
    vector<KeyPoint> keypoints1;
    akaze->detect( imagem1, keypoints1  );
    Mat descriptors1;
    akaze->compute( imagem1, keypoints1 , descriptors1 ); 
    gettimeofday(&end, NULL);
    seconds  = end.tv_sec  - start.tv_sec;
    useconds = end.tv_usec - start.tv_usec;
...
long AAKAZE_ANGULOS_VANT::tempo_1( )
{
  return tempo1;
}

long AAKAZE_ANGULOS_VANT::tempo_2( )
{
  return tempo2;
}

long AAKAZE_ANGULOS_VANT::tempo__casamento( )
{
  return tempo_casamento;
}

int AAKAZE_ANGULOS_VANT::pontos_1( )
{
  return pontos1;
}

int AAKAZE_ANGULOS_VANT::pontos_2( )
{
  return pontos2;
}

int AAKAZE_ANGULOS_VANT::pontos_casamento( )
{
  return pontos_matches; 
}

int AAKAZE_ANGULOS_VANT::pontos_casamento_total( )
{
  return pontos_matches_total;
}

vector<KeyPoint> AAKAZE_ANGULOS_VANT::key1( )
{
  return keypointsfil11;
}

vector<KeyPoint> AAKAZE_ANGULOS_VANT::key2( )
{
  return keypointsfil22;
}

Mat AAKAZE_ANGULOS_VANT::desc1( )
{
  return descriptorsfil11;
}

Mat AAKAZE_ANGULOS_VANT::desc2( )
{
  return descriptorsfil22;
}

vector< DMatch > AAKAZE_ANGULOS_VANT::keymatches( )
{
  return matches;
}
